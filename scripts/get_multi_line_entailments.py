...
for line in f:
    ...
    arr,d,d_inverse,middle_headers = tableHelper.createTable(table_json)
        ...
        for sentence in table_json["sentence_annotations"]:
            ...
            if ("total" in sentence) or ("average" in sentence) or cant_use:
                ...
                if len(highlighted_cell_rows)>1:
                    #print("rows 1")
                    options = {}
                    for r in highlighted_cell_rows.keys():
                        if r in usable_rows:
                            common_rows = []
                            flag = True
...
#with open('output_multi_entailments_v2_set2.csv', 'w') as file:
